package ch.maak.wz.communitylevels.communitylevels.server.level;
...
public class CreatorPageService implements ICreatorPageService {
  ...
  public CreatorTablePageData getCreatorTableData() {
    ...
    statement.append("SELECT CAST((@curRank := @curRank + 1) as CHAR(10)), "
      ...
      + "SUM(ATTEMPTS) / ( COUNT(LIKES) + 2 ) * 0.5 * CASE WHEN COUNT(LIKES) < 8 THEN 0.1 * COUNT(LIKES) + 0.2 ELSE 1 END AS POINTS_ATTEMPTS, "
      + "350 * ( 1 - SUM(WINS) / SUM(ATTEMPTS) ) * CASE WHEN COUNT(LIKES) < 8 THEN 0.1 * COUNT(LIKES) + 0.2 ELSE 1 END AS POINTS_DIFFICULTY, "
      + "SUM(LIKES) / ( COUNT(LIKES) + 2) * 10 * CASE WHEN COUNT(LIKES) < 8 THEN 0.1 * COUNT(LIKES) + 0.2 ELSE 1 END AS POINTS_LIKES "
    ...
    binds.append(addPageBind(CreatorTablePageData.CreatorTableRowData.points));
    binds.append(addPageBind(CreatorTablePageData.CreatorTableRowData.pointsAttempts));
    binds.append(addPageBind(CreatorTablePageData.CreatorTableRowData.pointsDifficulty));
    binds.append(addPageBindLast(CreatorTablePageData.CreatorTableRowData.pointsLikes));
