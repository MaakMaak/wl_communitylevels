...
public class CreatorPageService implements ICreatorPageService {
	...
	public CreatorTablePageData getCreatorTableData() {
		...
		statement.append("SELECT 1, CREATOR_NAME, CREATOR_ID, "
				+ "COUNT(CREATOR_ID) AS TOTAL_LEVELS, SUM(LIKES) AS TOTAL_LIKES, SUM(ATTEMPTS) AS TOTAL_ATTEMPTS, "
				+ "SUM(WINS) AS TOTAL_WINS, " + "SUM(WINS)/SUM(ATTEMPTS) * 100 AS WIN_RATE, "
				...
				+ "FROM COMMUNITY_LEVELS WHERE 1 = 1 " + "GROUP BY CREATOR_ID "
				+ "ORDER BY POINTS DESC, TOTAL_LIKES DESC, TOTAL_ATTEMPTS DESC");
